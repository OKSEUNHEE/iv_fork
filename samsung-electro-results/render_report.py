"""Create a self-contained Samsung Electro-Mechanics backtest report."""
...
def main() -> None:
    ...
    for index in range(59, len(rows) - 1):
        ...
        bucket["signals"] += 1
        bucket["correct"] += int((actual_return > 0) == signal_up)
        bucket["strategy"] *= 1 + (actual_return if signal_up else 0)
        bucket["benchmark"] *= 1 + actual_return
        daily.append((signal_up, actual_return))

    if not daily:
        raise RuntimeError("No rows available for report generation")

    strategy_equity, benchmark_equity = [1.0], [1.0]
    for signal_up, actual_return in daily:
    ...
    accuracy = sum((actual_return > 0) == signal_up for signal_up, actual_return in daily) / len(daily)
    up_days = sum(signal_up for signal_up, _ in daily)
...
<section class="note"><h2>해석 주의</h2><p>이 보고서의 ‘예측’은 종가 시점에 계산한 20일·60일 이동평균 기반의 다음 거래일 방향 신호입니다. 실제 종가와 다음 거래일 수익률은 검증 목적으로만 사용했으며, 신호 계산에는 해당 시점 이후 가격을 쓰지 않았습니다. 삼성전기 CSV와 LEAN 로컬 백테스트 결과를 기준으로 작성했으며, KRX 거래비용·세금·배당·액면분할·환율·시장충격은 반영하지 않았습니다. 과거 검증 결과는 미래 수익이나 매수 추천이 아닙니다.</p></section>
</body></html>"""
