...
OPERATION_DOCS: dict[str, tuple[str, str, str]] = {
    ...
    "/api/news/naver": ("DART·기업", "네이버 기업 뉴스 검색", "회사명을 기준으로 네이버 뉴스 검색 결과를 최신순으로 반환합니다. 네이버 Client ID와 Client Secret은 서버 환경변수에서만 사용하며, 결과는 5분간 캐시됩니다."),
    ...
    "/api/rag/search": ("RAG", "학습 문서 하이브리드 검색", "질문 임베딩으로 넓게 후보를 찾은 뒤 제목·핵심어 일치로 재정렬합니다. `score_threshold`는 벡터 후보의 최소 유사도입니다."),
