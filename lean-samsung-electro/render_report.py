"""Generate a self-contained HTML report for Samsung Electro-Mechanics (009150.KS) LEAN backtest results."""
...
def main() -> None:
    results_dir = Path("/home/okseunhee/investment-analysis/samsung-electro-results")
    full_json_path = results_dir / "SamsungElectroBuyAndHold.json"
    summary_json_path = results_dir / "SamsungElectroBuyAndHold-summary.json"
    output_html_path = results_dir / "samsung-electro-2024-report.html"
    ...
    close_raw = full_data.get("charts", {}).get("Samsung Electro-Mechanics", {}).get("series", {}).get("Close", {}).get("values", [])
    daily_closes = []
    ...
    net_profit = stats.get("Net Profit", "-3.430%")
    drawdown = stats.get("Drawdown", "6.600%")
...
<title>삼성전기 2024년 LEAN 백테스트 분석 보고서</title>
...
<body><h1>삼성전기(009150.KS) 2024년 백테스트 검증</h1><p>검증 기간: {start_date} ~ {end_date} (총 {len(daily_closes)} 거래일)</p>
...
<section><h2>삼성전기 일봉 종가 추이</h2><p>파랑: 삼성전기(009150.KS) 일별 종가 흐름 ({initial_close:,.0f}원 → {final_close:,.0f}원)</p>{svg_line(close_points, '#2563eb')}<p style="margin-top:20px;">회색: 포트폴리오 자산 가치(Strategy Equity) 변동 추이</p>{svg_line(equity_points, '#64748b')}</section>
...
<section class="note"><h2>해석 주의</h2><p>본 보고서는 QuantConnect LEAN 백테스트 엔진과 Yahoo Finance의 삼성전기 보통주(009150.KS) 일봉 데이터를 바탕으로 생성되었습니다. 초기 1,000,000원의 자본금으로 시작하여 1주 단순 매수 보유(Buy & Hold) 전략을 시뮬레이션한 결과입니다. KRX 실제 거래비용, 세금, 배당금 재투자, 액면분할, 환율 및 슬리피지 등은 반영되지 않았으며, 과거 데이터에 기반한 결과이므로 미래 수익을 보장하지 않습니다.</p></section>
</body></html>"""

    output_html_path.write_text(report_html, encoding="utf-8")
    print(f"Wrote HTML report to {output_html_path}")

    # Also write to render_report.py in lean-samsung-electro
    script_target = Path("/home/okseunhee/investment-analysis/lean-samsung-electro/render_report.py")
