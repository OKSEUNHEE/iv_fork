...
class SamsungElectroDaily(PythonData):
    """Yahoo Finance daily OHLCV CSV stored under LEAN's data directory."""

    def get_source(self, config, date, is_live):
        source = os.path.join(Globals.data_folder, "samsung_electro.csv")
...
class SamsungElectroBuyAndHold(QCAlgorithm):
    def initialize(self):
        start = date.fromisoformat(os.getenv("SAMSUNG_ELECTRO_START_DATE", "2024-01-01"))
        end = date.fromisoformat(os.getenv("SAMSUNG_ELECTRO_END_DATE", "2025-01-01"))
        self.set_start_date(start.year, start.month, start.day)
        self.set_end_date(end.year, end.month, end.day)
        self.set_cash(1_000_000)

        self.samsung_electro = self.add_data(SamsungElectroDaily, "009150", Resolution.DAILY).symbol
        self.bought = False

    def on_data(self, data):
        if self.samsung_electro not in data:
            return

        close = data[self.samsung_electro].value
        self.plot("Samsung Electro-Mechanics", "Close", close)

        if not self.bought:
            # Custom Data의 가격 단위로 1주를 매수해 엔진 주문 흐름을 확인합니다.
            self.market_order(self.samsung_electro, 1)
            self.bought = True
